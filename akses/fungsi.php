...
function tampil_fashionlain(){
	global $link;

	$query = "SELECT * FROM barang Where jenis_barang='fashion lain'";
	$result = mysqli_query($link,$query) or die ('Gagal menampilkan data barang');
	return $result;
}
...
function tambah_pembelian($nama_barang,$size,$harga,$nama_pembeli,$email_pembeli,$tlp_pembeli,$alamat_pembeli,$pos){
	global $link;

	$query = "INSERT INTO data_pembeli(nama_barang, size, harga, nama_pembeli, email_pembeli, tlp_pembeli, alamat_pembeli, pos) VALUES ('$nama_barang','$size','$harga','$nama_pembeli','$email_pembeli','$tlp_pembeli','$alamat_pembeli','$pos')";
	if (mysqli_query($link, $query)){
		return true;
	}else{
		return false;
